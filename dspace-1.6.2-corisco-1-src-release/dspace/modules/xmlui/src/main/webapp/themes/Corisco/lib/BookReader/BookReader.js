...
function BookReader() {
    ...
    //this.jToolbar = $('#BRtoolbar'); // j prefix indicates jQuery object
    this.toolbarSelector = '#BRtoolbar';
    this.toolbarParentSelector = '#BookReader';
    this.addToolbar = true;

//    this.toolbar = new Object;
//    this.toolbar.parentSelector = null;
//    this.toolbar.getHTMLContent = null;
//    this.toolbar.titles = null;
...
BookReader.prototype.toolbarHTMLContent = function() {
    var html =
        "<div id='BRtoolbar'>"
...
BookReader.prototype.toolbarTitles = { '.logo': 'Go to Archive.org',
                   '.zoom_in': 'Zoom in',
                   '.zoom_out': 'Zoom out',
...
BookReader.prototype.toolbarLabels = {};
...
BookReader.prototype.init = function() {
    ...
    if (1 == this.mode) {
        ...
        this.jumpToIndex(startIndex);
    } else if (this.constModeOCR == this.mode) {
        this.firstIndex = startIndex;
        this.prepareOCRView();
    } else {
        //this.resizePageView();
...
// drawLeafsTwoPage()
//______________________________________________________________________________
BookReader.prototype.drawOCR = function() {
    var scrollTop = $('#BRocrview').attr('scrollTop');
//    var scrollBottom = scrollTop + $('#BRocrview').height();

    this.setCurrentIndex(0);

    var viewHeight = $('#BRcontainer').attr('clientHeight');

    var iframe = document.createElement('iframe');
    iframe.src = this.OCRURL;
    $(iframe).css({
        id: 'OCRFrame',
        name: 'OCRFrame',
        width: '100%',
...
//        //$("#messagebar").append("; reductionFactors: "+reductionFactors.serializeArray());
//        ;
//    }
...
BookReader.prototype.switchMode = function(mode) {
    ...
    if (1 == mode) {
        ...
        this.prepareThumbnailView();
    } else if (mode == this.constModeOCR) {
        this.prepareOCRView();
    } else {
        // $$$ why don't we save autofit?
        this.twoPage.autofit = 'auto'; // Take zoom level from other mode
        this.twoPageCalculateReductionFactors();
...
BookReader.prototype.prepareOCRView = function() {
...
BookReader.prototype.getIdealSpreadSize = function(firstIndex, secondIndex) {
    ...
    var first = {
        height: 1.0*this._getPageHeight(firstIndex),
        width: 1.0*this._getPageWidth(firstIndex)
    }
    
    var second = {
        height: 1.0*this._getPageHeight(secondIndex),
        width: 1.0*this._getPageWidth(secondIndex)
    }
    ...
    if (ideal.height/ratio <= ideal.width) {
        ...
        //console.log('ideal.w = int(ideal.h/r): '+ideal.width);
    } else {
        //use width
        ideal.height = parseInt(ideal.width*ratio);
        //console.log('ideal.h = int(ideal.w/r): '+ideal.height);
    }
    
    // $$$ check this logic with large spreads
    ideal.reduce = ((first.height + second.height) / 2.0) / ideal.height;

//    console.log('(f.h('+first.height+') + s.h('+second.height+') ='
//        + parseFloat(first.height + second.height) +' / 2 = '
//        +((first.height + second.height) / 2.0)+' / i.h('+ideal.height+') = '
//        +((first.height + second.height) / 2.0) / ideal.height);
//    console.log('ideal.reduce='+ideal.reduce+'; current reduce='+this.reduce);
...
//    console.log("tlew("+totalLeafEdgeWidth+"),mlew("+maxLeafEdgeWidth+"),nw("+nativeWidth+"),nh("+nativeHeight+"),"+
//        "ssh("+spreadSize.height+"),ssw("+spreadSize.width+"),ssr("+spreadSize.reduce+")");
...
BookReader.prototype.currentIndex = function() {
    ...
    if (this.mode == this.constMode1up || this.mode == this.constModeThumb || this.mode == this.constModeOCR) {
...
BookReader.prototype.printPage = function() {
    ...
    var htmlToPrint = this.getPrintFrameContent(indexToPrint);
    var w = window.open();
    try {
        w.document.open();
...
//    window.open(this.getPrintURI(), 'printpage', 'width=400, height=500, resizable=yes, scrollbars=no, toolbar=no, location=no');
    ...
    htmlStr =  '<p style="text-align:center;"><b><button class="BRicon print rollover" onclick="window.frames[0].focus(); window.frames[0].print(); return false;"/><a href="javascript:void(0);" onclick="window.frames[0].focus(); window.frames[0].print(); return false;">Clique aqui para imprimir</a></b></p>';
...
//    htmlStr += '<a href="#" onclick="br.updatePrintFrame(-1); return false;">Prev</a> <a href="#" onclick="br.updatePrintFrame(1); return false;">Next</a>';
    htmlStr +=   '</p>';
    htmlStr += '</div>';
    htmlStr += '<p style="text-align:center;"><a href="" onclick="br.printPopup = null; $(this.parentNode.parentNode).remove(); return false">Fechar</a></p>';
    htmlStr += '</div>';
...
BookReader.prototype.autoToggle = function() {
    ...
    if (null == this.autoTimer) {
        ...
        $(this.toolbarSelector).find('.play').hide();
        $(this.toolbarSelector).find('.pause').show();
...
BookReader.prototype.autoStop = function() {
    if (null != this.autoTimer) {
        ...
        this.flipSpeed = 'fast';
        $(this.toolbarSelector).find('.pause').hide();
        $(this.toolbarSelector).find('.play').show();
        this.autoTimer = null;
    }
}
...
BookReader.prototype.initToolbar = function(mode, ui) {
    ...
    if (this.addToolbar == true)
        $(this.toolbarParentSelector).append(this.toolbarHTMLContent());
    ...
    var jToolbar = $(this.toolbarSelector); // j prefix indicates jQuery object
    ...
    if ('rl' == this.pageProgression) { // RTL
        this.toolbarTitles['.book_leftmost'] = this.toolbarTitles['.book_bottom'];
        this.toolbarTitles['.book_rightmost'] = this.toolbarTitles['.book_top'];
    } else { // LTR
        this.toolbarTitles['.book_leftmost'] = this.toolbarTitles['.book_top'];
        this.toolbarTitles['.book_rightmost'] = this.toolbarTitles['.book_bottom'];
    }
                  
    for (var icon in this.toolbarTitles) {
        jToolbar.find(icon).attr('title', this.toolbarTitles[icon]);
    }

    for (var label in this.toolbarLabels) {
        jToolbar.find(label).text(this.toolbarLabels[label]);
    }
...
BookReader.prototype.switchToolbarMode = function(mode) {
    $(this.toolbarSelector).find('.modos_visualizador').find('.icone_barra').removeClass('selected');

    if (mode != this.constModeOCR) {
        $(this.toolbarSelector).find('.zoom').css('visibility', 'visible');
        $(this.toolbarSelector).find('.paginacao').css('visibility', 'visible');
        $(this.toolbarSelector).find('.resultados-pagina').css('visibility', 'visible');
    }
    if (1 == mode) {
        // 1-up
        $(this.toolbarSelector).find('.one_page_mode').addClass('selected');

        $(this.toolbarSelector).find('.BRtoolbarzoom').show().css('display', 'inline');
        $(this.toolbarSelector).find('.BRtoolbarmode2').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode3').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode1').show().css('display', 'inline');
    } else if (2 == mode) {
        // 2-up
        $(this.toolbarSelector).find('.two_page_mode').addClass('selected');

        $(this.toolbarSelector).find('.BRtoolbarzoom').show().css('display', 'inline');
        $(this.toolbarSelector).find('.BRtoolbarmode1').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode3').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode2').show().css('display', 'inline');
    } else if (this.constModeOCR == mode) {
        // OCR
        $(this.toolbarSelector).find('.ocr_mode').addClass('selected');

        $(this.toolbarSelector).find('.zoom').css('visibility', 'hidden');
        $(this.toolbarSelector).find('.paginacao').css('visibility', 'hidden');
        $(this.toolbarSelector).find('.resultados-pagina').css('visibility', 'hidden');

//        $(this.toolbarSelector).find('.BRtoolbarzoom').hide();
//        $(this.toolbarSelector).find('.BRtoolbarmode1').hide();
//        $(this.toolbarSelector).find('.BRtoolbarmode3').hide();
//        $(this.toolbarSelector).find('.BRtoolbarmode2').hide();
    } else {
        // 3-up
        $(this.toolbarSelector).find('.thumbnail_mode').addClass('selected');

        $(this.toolbarSelector).find('.BRtoolbarzoom').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode2').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode1').hide();
        $(this.toolbarSelector).find('.BRtoolbarmode3').show().css('display', 'inline');
    }
}
...
BookReader.prototype.paramsFromFragment = function(urlFragment) {
    ...
    } else if ('ocr' == urlHash['mode']) {
        params.mode = this.constModeOCR;
    }
...
BookReader.prototype.fragmentFromParams = function(params) {
    ...
    if ('undefined' != typeof(params.mode)) {    
        ...
        } else if (params.mode == this.constModeOCR) {
            fragments.push('mode', 'ocr');
        } else {
...
// _getOCRURI
//--------
// Returns the book's OCR URI, which should point to text content.
//BookReader.prototype._getOCRURI= function() {
//    return this.getOCRURI();
//}
