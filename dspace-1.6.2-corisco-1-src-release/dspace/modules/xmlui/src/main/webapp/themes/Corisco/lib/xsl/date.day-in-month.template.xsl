...
<xsl:template name="day-in-month">
   ...
   <xsl:variable name="day">
      <xsl:if test="not(string($timezone)) or                     $timezone = 'Z' or                      (substring($timezone, 2, 2) &lt;= 23 and                      substring($timezone, 5, 2) &lt;= 59)">
         ...
         <xsl:choose>
            <xsl:when test="starts-with($dt, '---') and                             $dt-length = 5 and                             substring($dt, 4, 2) &lt;= 31">
               <xsl:value-of select="substring($dt, 4, 2)"/>
            </xsl:when>
            <xsl:when test="number(substring($dt, 1, 4)) and                             substring($dt, 5, 1) = '-' and                             substring($dt, 6, 2) &lt;= 12 and                             substring($dt, 8, 1) = '-' and                             substring($dt, 9, 2) &lt;= 31 and                             ($dt-length = 10 or                              (substring($dt, 11, 1) = 'T' and                               substring($dt, 12, 2) &lt;= 23 and                               substring($dt, 14, 1) = ':' and                               substring($dt, 15, 2) &lt;= 59 and                               substring($dt, 17, 1) = ':' and                               substring($dt, 18) &lt;= 60))">
